...
import { addStudentPayment, getStudentPayments } from "apis/student";
...
const validationSchema = Yup.object({
	...
	applicationId: Yup.string().nullable().required("Required"),
...
const Payments = ({ studentId }) => {
	...
	useEffect(() => {
		...
		getApplications({ studentId }).then(({ data }) => setApplicationList(data));
	}, []);
	...
	const onSubmit = values => {
		...
		addStudentPayment({ studentId, data: reqData })
			.then(() => {
				_fetchData();
				setOpen(false);
			})
										...
										{props => {
											...
											return (
												<DropdownWithSearch
													...
													options={applicationList?.map(({ id }) => id)}
													renderOption={(props, option) => {
														const selectedOption = applicationList?.filter(
															({ id }) => id === option
														)[0];
													...
													getOptionLabel={option => {
														...
														return selectedOption?.applicationId;
													}}
													handleOnChange={({ key, value }) => {
														...
														form.setFieldValue(
															"currency",
															value ? selectedOption.currency.symbol : null
														);
													}}
													inputProps={{
